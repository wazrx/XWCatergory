...
/**
 1、[NSDate date]获取的时间是GMT时间
 2、打印的时候使用的NSDateFormatter 如果不人为设置timeZone，其时区为当前系统的时区，所以如果格式化字符串当前时间就会把GMT时间转换为系统时间，打印出正确的结果
 3、关于时间戳，将时间戳转换为date之后，有可能这个date应该是GMT+8也就是东八区的时间，服务器那边是中文的，但是转换后默认的时区就是GMT时区，这是使用格式化，系统仍按照GMT时间处理，如果此时系统是东八区那么处理后的时间会增加8个小时，所以当我们将时间戳转出date后，需要根据服务器那边的时区增减对应的时间即可
 */
...
- (NSDate *)xw_changeUTCDateToGMTDate{
...
+ (NSDate *)xw_dateWithTimestamp:(NSString *)timestamp timeZoneName:(NSString *)timeZoneName{
    NSDate *confromTimesp = [NSDate dateWithTimeIntervalSince1970:[timestamp doubleValue]];
    NSDate *date = [confromTimesp dateByAddingTimeInterval:480 * 60];
    return [date xw_changeDateWithTimeZoneName:timeZoneName];
}

+ (NSDate *)xw_changeCurrentTimeToTimeZoneDate:(NSTimeZone *)timeZone{
    NSTimeZone *sysetemZone = [NSTimeZone systemTimeZone];
    NSTimeInterval timeSpacing = [timeZone secondsFromGMT] - [sysetemZone secondsFromGMT];
    return [[NSDate date] dateByAddingTimeInterval:timeSpacing];
}

- (NSDate *)xw_changeDateWithTimeZoneName:(NSString *)timeZoneName{
    NSTimeZone *sysetemZone = [NSTimeZone systemTimeZone];
    NSTimeInterval timeSpacing = [[NSTimeZone timeZoneWithName:timeZoneName] secondsFromGMT] - [sysetemZone secondsFromGMT];
    return [self dateByAddingTimeInterval:timeSpacing];
}
